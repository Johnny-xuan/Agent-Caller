---
name: agent-caller
description: Create and coordinate durable sub-agents backed by Claude Code or Codex. Use when Codex should delegate work to an independent agent, assemble named team roles, ask another model for an opinion, run agents in parallel, continue a multi-turn agent conversation, inspect prior agent output, release an idle agent, stop active work, or restore a previously released or interrupted agent.
---
...
Before the first delegation to each provider in the current Codex task:
...
The MCP server cannot enforce this because MCP does not expose the parent Codex
task identity. See [models-and-effort.md](references/models-and-effort.md) for
...
4. Default to `profile=trusted` for ordinary personal local work. Choose
   `guarded` for supervised writes and `observer` for read-only analysis.
5. Express review, rescue, adversarial analysis, implementation, or research in
...
for Codex to judge and verify, not as the final authority.
...
An Agent's creation policy is its authority ceiling. A Run may narrow it but
cannot widen it. The parent Codex sandbox is not inherited over MCP.
